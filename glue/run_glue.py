...
# Source: https://github.com/McGill-NLP/bias-bench/tree/main
...
import sys
sys.path.append("./bias-evaluation/bias_bench/model")
...
class ModelArguments:
    ...
    lora_dim: int = field(
        default=4,
        metadata = {"help": "LoRA dimension; 0 means LoRA is disabled"})
    lora_dropout: float = field(
        default=0.0,
        metadata = {"help": "Dropout probability for LoRA layers"})
    lora_alpha: int = field(
        default=32,
        metadata = {"help": "LoRA attention alpha"})
...
def main():
    ...
    if model_args.model == "DPLoRAGPT2ForSequenceClassification" or model_args.model == "LoRAGPT2ForSequenceClassification" or model_args.model == "DPLoRAptGPT2ForSequenceClassification":
        print("load lora model")
        model = getattr(models, model_args.model)(
            model_args.model_name_or_path,  model_args.lora_dim, model_args.lora_alpha, model_args.lora_dropout, config=config, **kwargs
        )
        print("transformer.h.21.attn.c_attn.lora_A.weight:")
