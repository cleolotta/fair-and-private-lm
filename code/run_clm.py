...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="na",
        help="The objective the model should be trained on"
            "cda - counterfactual data augmentation for debiasing"
            "dp - differential privacy"
            "dp_cda - debiasing and differential privacy"
            "na - no objective, only further training",
        choices=["cda","dp","dp_cda","na"]
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
...
def main():
    ...
    folder_name = f"objective_{args.objective}_lora_{args.lora_dim}_layer_{args.train_layer_n_only}_ref_{args.do_ref_model}_maxlen_{args.block_size}_model_{args.model_name_or_path}_lr_{args.learning_rate}_epoch_{args.num_train_epochs}_trba_{args.per_device_train_batch_size}_acc_{args.gradient_accumulation_steps}_evba{args.per_device_eval_batch_size}"
    ...
    if (args.objective == "cda" or args.objective == "dp_cda") and (args.mia_train_file is None or args.mia_validation_file is None):
        raise argparse.ArgumentTypeError('Original/not-augmented data needed for membership inference attack')
    ...
    if args.mia_train_file is not None and args.objective != "na" and args.objective != "dp":
        data_files["mia_train"] = args.mia_train_file
    if args.mia_validation_file is not None and args.objective != "na" and args.objective != "dp":
...
#        logger.warning("You are instantiating a new config instance from scratch.")
    ...
    if (args.objective == "cda" or args.objective == "dp_cda") and args.mia_train_file is not None:
        mia_train_dataset = lm_datasets["mia_train"]
    elif (args.objective == "cda" or args.objective == "dp_cda") and args.mia_train_file is None:
        print("different dataset to measure mia on needed for augmentation objective")
        
    if (args.objective == "cda" or args.objective == "dp_cda") and args.mia_validation_file is not None:
        mia_eval_dataset = lm_datasets["mia_validation"]
    elif (args.objective == "cda" or args.objective == "dp_cda") and args.mia_validation_file is None:
        print("different dataset to measure mia on needed for augmentation objective")
    ...
    if args.objective == "cda" or args.objective == "dp_cda":
    ...
    if args.objective == "cda" or args.objective == "dp_cda":
        ...
        mia_train_dataloader = train_dataloader
        mia_train_dataset = train_dataset # for cases in which reference model is used for same data 
        mia_eval_dataloader = eval_dataloader
        mia_eval_dataset = eval_dataset # for cases in which reference model is used for same data 
    ...
    if args.objective == "dp" or args.objective == "dp_cda":
        ...
        privacy_engine = opacus.PrivacyEngine(module=model, 
            ...
            max_grad_norm=args.per_sample_max_grad_norm, noise_multiplier=noise_multiplier, target_delta=1.0/len(train_dataset)
        )
